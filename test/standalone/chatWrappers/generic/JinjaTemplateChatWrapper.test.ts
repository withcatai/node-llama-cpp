...
import {LlamaText, SpecialTokensText} from "../../../../src/utils/LlamaText.js";
...
describe("JinjaTemplateChatWrapper", () => {
        ...
        test("mistral template", () => {
            ...
            expect(contextText).toMatchInlineSnapshot(`
              LlamaText([
                ...
                "Hello!",
                new SpecialToken("EOS"),
                new SpecialTokensText("[TOOL_CALLS][{"name": "func2", "arguments": {"message": "Hello", "feeling": "good", "words": 1}, "id": "fc_1_0001"}]"),
                new SpecialToken("EOS"),
                new SpecialTokensText("[TOOL_RESULTS]{"content": "),
                "{"yes": true, "message": "ok"}",
                new SpecialTokensText(", "call_id": "fc_1_0001"}[/TOOL_RESULTS]"),
                new SpecialToken("EOS"),
    ...
    describe("thought segment extraction", () => {
        test("Qwen 3.6 resolves thinking segment settings properly", {timeout: 1000 * 60 * 60 * 2}, async () => {
            const chatWrapper = new JinjaTemplateChatWrapper({
                template: qwen3_6Template
            });
        ...
        test("basic template resolves thinking segment settings properly", {timeout: 1000 * 60 * 60 * 2}, async () => {
            const template = (
                "{%- for m in messages %}" +
                    "{{- '<|im_start|>' + m.role + '\\n' }}" +
